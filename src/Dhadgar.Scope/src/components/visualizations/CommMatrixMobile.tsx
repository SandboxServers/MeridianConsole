...
export function CommMatrixMobile() {
  ...
  const filteredCells = currentRow?.cells.filter((c) => {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {data.rows.map((r) => (
          <option key={r.from} value={r.from}>{r.from}</option>
        ))}
      </select>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${
            protocolFilter === '' ? 'bg-white/20 text-white' : 'bg-white/5 text-white/70 hover:bg-white/10'
          }`}
        >
          All
        </button>
        {protocols.map((p) => (
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${
              protocolFilter === p ? 'bg-white/20 text-white' : 'bg-white/5 text-white/70 hover:bg-white/10'
            }`}
          >
            {p}
          </button>
        ...
        {filteredCells.length > 0 ? (
          filteredCells.map((cell) => (
            ...
            >
              ...
              <span className={`rounded-lg px-2 py-1 text-xs font-semibold ${protocolColors[cell.value] || 'bg-gray-600'}`}>
                {cell.value}
              </span>
            </div>
          ))
      ...
      {currentRow && (
        <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-white/60">
          {selectedSource} has {currentRow.cells.filter((c) => c.value !== '-').length} outbound connections
        </div>
      )}
    </div>
  );
