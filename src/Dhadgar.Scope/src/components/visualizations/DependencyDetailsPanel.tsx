      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <span className={`inline-block h-2 w-2 rounded-full ${layerColors[selected.layer] || 'bg-gray-600'}`} />
            <span className="text-xs text-white/60 capitalize">{selected.layer} layer</span>
            {selected.port && (
              <span className="text-xs text-white/60">• Port {selected.port}</span>
            )}
          </div>
        </div>
        <button
        ...
        <div className="mt-4">
          ...
          <ul className="mt-2 space-y-1">
            {selected.endpoints.map((e, i) => (
              <li key={i} className="text-sm font-mono text-white/70">{e}</li>
            ))}
          </ul>
        </div>
      )}
