...
public sealed class GetRoleCommand
{
    public static async Task<int> ExecuteAsync(string roleId, string? orgId, CancellationToken ct)
    {
        ...
        try
        {
            var response = await identityApi.GetRoleAsync(orgId, roleId, ct);
