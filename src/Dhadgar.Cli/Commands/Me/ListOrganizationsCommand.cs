...
public sealed class ListOrganizationsCommand
{
    public static async Task<int> ExecuteAsync(CancellationToken ct)
    {
        ...
        try
        {
            var response = await identityApi.GetMyOrganizationsAsync(ct);
            IdentityCommandHelpers.WriteJson(response.Organizations);
            return 0;
        }
